		...
		public void run() {
			...
			while (mShouldRunning) {
				try {
					...
					//Loger.disk_log("调试日志", "read后:"+StringTool.byteArrayToString(btAryBuffer, 0, btAryBuffer.length), "M10_U8");
					if (nLenRead > 0) {
	...
	private int sendMessage(byte[] btArySenderData) {
		...
		try {
			synchronized (mOutStream) { // 防止并发
				Loger.disk_log("新增日志", "write前"+ StringTool.byteArrayToString(btArySenderData, 0, btArySenderData.length), "M10_U8");
